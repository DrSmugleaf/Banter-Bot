    ...
    async.whilst(
      ...
      function(e, lastMessageID, lines) {
        ...
        if(!(lines && lastMessageID)) {
          return msg.reply(constants.responses.MARKOV.MISSING[msg.language](subject))
        }

        markovChain.seed(lines.join(os.EOL), function() {
          const markovAnswer = markovChain.fill(markovChain.pick(), 25).join(" ").replace("@", "@ ")
          const answer = constants.responses.MARKOV.ANSWER[msg.language](subject, markovAnswer)
